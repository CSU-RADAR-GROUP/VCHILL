...
public class ViewPlotMethodPPI extends ViewPlotMethod {
    ...
    @Override
    public void plotMapServerOverlay(final Graphics g) {
    ...
    @Override
    public void plotMap(final Graphics g) {
        ...
        for (MapInstruction instr : vc.getMap()) {
            ...
            if (prevInstr != null && prevInstr.getType() == Shape.CIRCLE) { //special case
                ...
                case POINT:
                    ...
                    break;
                default:
                    throw new Error("Don't know how to handle map shape " + instr.getType());
            }
     ...
     */
    @Override
    protected int getPixelsFromKm(final double distance) {
    ...
    @Override
    public double getElevationInKm(final double elevation, final double range) {
    ...
    public void export(final ZipOutputStream zip) throws IOException {
        ...
        try {
            out = new OutputStreamWriter(zip, "UTF-8");
