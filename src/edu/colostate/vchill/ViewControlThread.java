...
public class ViewControlThread implements Runnable
{
    ...
    /**
     * This thread class will handle all of the plotting requests that the
     * user makes. It takes messages in through a shared <code>ControlSyncQueue</code>
     * and references the Controller class only for the <code>getRay</code> method.
     *
     * @param queue The shared message queue to recieve plotting requests.
     * @param controller This is the controller class that has the connections
     * to the data retrieval methods, the only method that should be
     * used is getRay()
     */
    public ViewControlThread (final ControlSyncQueue<ControlMessage> queue, final Controller controller)
    {
    ...
    /**
     * This is the main loop where the thread attempts to get the first
     * message that is on the InputData class. If not found it will wait
     * until something is added. It then goes into the plotting method.
     * NOTE: There may need to be some form of file checking to leave
     * sweeps behind instead of clearing the buffers.
     */
    public void run ()
    {
        while (true) {
            ...
            if (currMessage.getURL() == null || currMessage.getDir() == null ||
                currMessage.getFile() == null || currMessage.getSweep() == null) {
                ...
                ChillDefines.REALTIME_SWEEP.equals(currMessage.getSweep())) {
                continue; //plotted as it comes in; don't need to do anything
            } else { //good to go?
                String filter = config.getScanFilter();
                int sweepNum = 0;
                try { sweepNum = Integer.parseInt(currMessage.getSweep().split(" ")[1]); }
                catch (Exception e) { System.out.println("Bad filter"); } //ignore, can be caused by "Nothing to display"
                int sweepFilter = config.getAutosaveTilt();
                if ((filter == null || currMessage.getFile().contains(filter)) && //filename filter
                        (sweepFilter == 0 || sweepFilter == sweepNum)) { //sweep filter
                    newPlot();
    ...
    {
	        final ViewPlotWindow[] plotWins = this.plots.toArray(new ViewPlotWindow[this.plots.size()]);
    ...
    /**
     * This method has three major parts.  They would be broken up but for
     * all of the data passing that would be required.  First it finds the
     * list of all the active windows.  It then tries to get a ray; if not
     * found it will sleep and try a few more times.  Once the ray is
     * found, it asks for the threshold field for the sake of interleaving the
     * fields.  Finally it sends the data to the window for actual plotting.
     * Note: This is also where stop is implemented; the main while loop 
     * looks for a stop action in the ControlSyncQueue queue and if one
     * is found it breaks out of the while loops.
     *
    ...
    private void newPlot ()
    {
        ...
        { //request data & ensure a window is open
            ...
            if (thresh != null) allTypes.add(thresh);
            ...
            //sm.clear(); //clear data type list; will be recreated from metadata stream
//System.out.println("Requesting " + currMessage.setType(allTypes));
			System.out.println("Requesting sweep");
            controller.requestSweep(currMessage, allTypes); //always request to ensure we get datatype list
            if (num_windows == 0) return; //nothing to display
        }
		System.out.println("Waiting for sweep to finish");
		while( !controller.isSweepDone(currMessage) ) {
			try {Thread.sleep(250);} catch (InterruptedException e) {} //ensure fetchthread gets a chance to see request
		}
		
        {//now that all the types are coming in, start plotting
			System.out.println("Starting plotting");
            wm.setPlotting(true);
            //final ControlMessage meta = currMessage.setType(ChillDefines.META_TYPE);
	    for (int metaNum = 0; /*check in loop*/; ++metaNum) {
			if (queue.stopping()) break;
			ChillHeader header = (ChillHeader)controller.getRayWait(currMessage, ChillDefines.META_TYPE, metaNum);
			...
			if (header.header.recordType == ChillDefines.GEN_MOM_DATA) {
			    ...
			    //System.out.println("requested 0x" + Long.toHexString(dh.requestedData));
			    //System.out.println("available 0x" + Long.toHexString(dh.availableData));
			    //System.out.println("not avail 0x" + Long.toHexString(dh.requestedData & ~dh.availableData));
			    break;
			}
	    }
	    System.out.println("Checking availability");
            ...
            final ViewAScopeWindow[] ascopeWins = this.ascopes.toArray(new ViewAScopeWindow[this.ascopes.size()]);
            final String[][] ascopeTypes = new String[ascopeWins.length][2];
            for (int i = 0; i < ascopeWins.length; ++i) {
                ascopeTypes[i][0] = this.checkAvailability(ascopeWins[i].getType());
                ascopeTypes[i][1] = this.checkAvailability(ascopeWins[i].getSecondary());
            }

            final NumDumpWindow[] numdumpWins = this.numdumps.toArray(new NumDumpWindow[this.numdumps.size()]);
            final String[] numdumpTypes = new String[numdumpWins.length];
            for (int i = 0; i < numdumpWins.length; ++i) {
                numdumpTypes[i] = this.checkAvailability(numdumpWins[i].getType());
            }

            String threshold = this.checkAvailability(config.getThresholdType());
            ...
            while (true) {
                ...
                switch (header.header.recordType) {
                    case ChillDefines.GEN_MOM_DATA:
                        //System.out.println("0x" + Long.toHexString(((ChillDataHeader)header).requestedData));
                        //System.out.println("0x" + Long.toHexString(((ChillDataHeader)header).availableData));

                        threshRay = threshold == null ? null : getRayWait(currMessage, threshold, rayNum);
                        for (int i = 0; i < plotWins.length; ++i) { //get rays / detect end
                            ...
                            if (plotCurrRays[i] != null) { //null => not available => sweep done?
                                ...
                                plotNextRays[i] = getRayWait(currMessage, plotTypes[i], rayNum + 1);
                                plotWins[i].plot(plotPrevRays[i], plotCurrRays[i], plotNextRays[i], threshRay);
                            }
                        }
                        for (int i = 0; i < ascopeWins.length; ++i) { //get rays / detect end
                            if (ascopeTypes[i][0] != null) ascopeRays[i][0] = getRayWait(currMessage, ascopeTypes[i][0], rayNum);
                            if (ascopeRays[i][0] != null) { //null => not available => sweep done?
                                if (!rangeSet) {
                                    config.setMaxPlotRange(ascopeRays[i][0].getEndRange());
                                    rangeSet = true;
                                }
                                if (ascopeTypes[i][1] != null) ascopeRays[i][1] = getRayWait(currMessage, ascopeTypes[i][1], rayNum);
                            }
                        }
                        for (int i = 0; i < numdumpWins.length; ++i) { //get rays / detect end
                            if (numdumpTypes[i] == null) continue; //seems to be unavailable
                            numdumpRays[i] = getRayWait(currMessage, numdumpTypes[i], rayNum);
                            if (numdumpRays[i] != null) { //null => not available => sweep done?
                                if (!rangeSet) {
                                    config.setMaxPlotRange(numdumpRays[i].getEndRange());
                                    rangeSet = true;
                                }
                        ...
                        if (rayNum % config.getRayStep() == 0) {
                            for (ViewPlotWindow win : plotWins) win.rePlotDrawingArea();
                            for (int i = 0; i < ascopeWins.length; ++i) {
                                ascopeWins[i].plot(ascopeRays[i][0], ascopeRays[i][1]);
                                ascopeWins[i].repaint(ascopeWins[i].getVisibleRect());
                            }
                            Thread.yield();
                        }
...
//System.out.println("Aircraft data found");
		    ...
		    case ChillDefines.OLD_EXT_TRACK_DATA: //extended aircraft info
                    ...
                    case ChillDefines.NEW_EXT_TRACK_DATA: //extended aircraft info
                        ...
                        break;
                    case ChillDefines.HSK_ID_PROCESSOR_INFO: //processor information
                        ChillProcessorInfo procinf = (ChillProcessorInfo)header;
                        config.setMaxPlotRange(procinf.range_stop_km);
                        rangeSet = true; //don't bother calculating from ray headers
                        break;
                    case ChillDefines.HSK_ID_END_NOTICE: //end of sweep/volume - archive doesn't send?
System.out.println("viewcontrolthread: end notice");
            ...
            for (int i = 0; i < plotWins.length; ++i) {
                ...
                if (ray != null) { //saving possible
                    if (config.isSaveAllEnabled() || //always save
                        ...
                        if (config.isImageAutosaveEnabled()) { //image save enabled
    ...
    /**
     * This method makes the request from the Control object for the actual
     * ray data.
     *
     ...
     * @return A Ray of data, or null to signify termination or end of the requested volume.
     */
    public Ray getRay (final ControlMessage msg, final String type, final int rayIndex)
    {
        try {
            return msg == null ? null : (Ray)controller.getRay(msg, type, rayIndex);
        } catch (Exception e) {
    ...
    /**
     * This is the same as the getRay method with one major difference.  
     * If the ray is not yet available, it will wait for it to come in or
     * for the sweep to be marked complete.
     *
     ...
     */
    public Ray getRayWait (final ControlMessage msg, final String type, final int rayIndex)
    {
        return msg == null ? null : (Ray)controller.getRayWait(msg, type, rayIndex);
    }

    /**
     * @param msg ControlMessage describing the ray that is being requested.
     * @param rayIndex The index of the requested ray.
     * @param timeout the time (in ms) to wait for the ray to come in if it is not yet available
     * @return The ray of data that was requested.
     */
    public Ray getRayWait (final ControlMessage msg, final String type, final int rayIndex, final long timeout)
    {
        return msg == null ? null : (Ray)controller.getRayWait(msg, type, rayIndex, timeout);
    }
}
