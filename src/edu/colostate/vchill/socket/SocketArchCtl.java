...
public final class SocketArchCtl {
    ...
    public SocketArchCtl(final byte[] data, int offset) {
        ...
        this.archMode = Command.values()[SocketUtil.readInt(data, offset)];
        offset += ChillDefines.INT_BYTE_SIZE;
        this.startSweep = SocketUtil.readShort(data, offset);
        offset += ChillDefines.SHORT_BYTE_SIZE;
        this.rayStep = SocketUtil.readShort(data, offset);
        offset += ChillDefines.SHORT_BYTE_SIZE;
        this.sweepLow = SocketUtil.readShort(data, offset);
        offset += ChillDefines.SHORT_BYTE_SIZE;
        this.sweepHigh = SocketUtil.readShort(data, offset);
        offset += ChillDefines.SHORT_BYTE_SIZE;
        this.extraDelay = SocketUtil.readInt(data, offset);
        offset += ChillDefines.INT_BYTE_SIZE;
        this.inFile = new byte[IN_FILE_LENGTH];
    ...
    public Command getArchMode() {
        return archMode;
    }

    public short getStartSweep() {
        return startSweep;
    }

    public short getRayStep() {
        return rayStep;
    }

    public short getSweepLow() {
        return sweepLow;
    }

    public short getSweepHigh() {
        return sweepHigh;
    }
