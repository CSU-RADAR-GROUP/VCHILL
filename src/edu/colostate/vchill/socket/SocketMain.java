...
System.out.println("session id = " + this.sessionID);
    ...
    private void sendDataHandshake () throws IOException
    {
        ...
        try { Thread.sleep(250); } //wait to see if data is coming in
    ...
    protected SocketResponse readResponse () throws IOException
    {
		...
		DialogUtil.showErrorDialog("Kicked from server " + this.url + "!");
            ...
            switch (response.getStatus()) {
                case REQUEST_ERROR: case OPEN_ERROR: case FORMAT_ERROR: //handle server error codes
    ...
    {
        this.dataIn.close();this.dataIn = null;
        this.dataOut.close();this.dataOut = null;
        this.dataSocket.close();this.dataSocket = null;
        this.controlIn.close();this.controlIn = null;
        this.controlOut.close();this.controlOut = null;
    ...
    public synchronized void getSweep (final ControlSyncQueue<TypedControlMessage> commands, final CacheMain cache) throws IOException
    {
        ...
        for (String type : command.types) {
            ...
            if (requestTypes.size() == ChillDefines.MAX_PER_CHANNEL) break; //allow at most MAX_PER_CHANNEL data types (this is the server's limit)
        }
    ...
    private void clearChannels () throws IOException
    {
        ...
        while (this.dataIn.available() + this.controlIn.available() > 0) {
            ...
            try { Thread.sleep(100); } //wait to see if data is still coming in
            catch (InterruptedException ie) {}
        }
    }
