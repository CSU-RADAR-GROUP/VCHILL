...
public final class ViewControl
{
    ...
    {
		ControlMessage currMsg = this.getControlMessage();
        try { return this.controller.isConnected(currMsg.setURL(url)); }
    ...
    {
		ControlMessage currMsg = this.getControlMessage();
        try { return this.controller.isConnected(currMsg); }
        catch (Exception e) { return false; }
    }
    ...
    public synchronized void rePlot ()
    {
        ...
        switch (config.getPlottingMode()) {
            case Ray: this.plotRay(this.lastRayPlotted); break;
            default: this.plotQueue.add(this.controlMsg);
        }
    }
    ...
    public void plotRay (final int index)
    {
        ...
        if (config.isRealtimeModeEnabled() || config.getPlottingMode() == Mode.Ray) for (final ViewPlotWindow win : wm.getPlotList()) {
            Ray prev = null;
            Ray next = null;
            Ray thresh = null;
            try {
                prev = (Ray)this.controller.getRay(currMsg, win.getType(), index - 1);
            } catch (ArrayIndexOutOfBoundsException aioobe) { } //ignore - previous not vital
            try {
                next = (Ray)this.controller.getRay(currMsg, win.getType(), index + 1);
            } catch (ArrayIndexOutOfBoundsException aioobe) { } //ignore - next not vital
            try {
                thresh = (Ray)this.controller.getRay(currMsg, config.getThresholdType(), index);
            } catch (ArrayIndexOutOfBoundsException aioobe) { } //ignore - threshold not vital
            try {
        ...
        for (final ViewAScopeWindow win : wm.getAScopeList()) {
            ...
            } catch (ArrayIndexOutOfBoundsException aioobe) { } //ignore - secondary not vital
            try {
    ...
    public synchronized void sweepDone ()
    {
		if (!this.plotQueue.stopping()) {
			switch (config.getPlottingMode()) {
				case Volume: ViewFileBrowser.getInstance().getActions().selectNext(); break;
				case Continuous: ViewFileBrowser.getInstance().getActions().selectNextAcrossBounds(); break;
			}
		}
    }
    ...
    public synchronized void createBookmark (final ControlMessage msg)
    {
        ...
        bmc.addBookmark(BookmarkControl.USER_PREFIX + (String)category.getSelectedItem(), name.getText(), bookmark);
    }
