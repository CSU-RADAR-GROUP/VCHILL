...
public class SigmetProductRaw {
    ...
    public SigmetProductRaw(DataInputStream dis) {
        ...
        int total_vars = Integer.bitCount((int) (top_ingest_header.getAtask_configuration().getAtask_dsp_info().getCurrent_data_type().getMask_word_0()));

        total_vars += Integer.bitCount((int) (top_ingest_header.getAtask_configuration().getAtask_dsp_info().getCurrent_data_type().getMask_word_1()));
        total_vars += Integer.bitCount((int) (top_ingest_header.getAtask_configuration().getAtask_dsp_info().getCurrent_data_type().getMask_word_2()));
        total_vars += Integer.bitCount((int) (top_ingest_header.getAtask_configuration().getAtask_dsp_info().getCurrent_data_type().getMask_word_3()));
        total_vars += Integer.bitCount((int) (top_ingest_header.getAtask_configuration().getAtask_dsp_info().getCurrent_data_type().getMask_word_4()));

        System.out.println("Data Mask Word 1:" + (int) (top_ingest_header.getAtask_configuration().getAtask_dsp_info().getCurrent_data_type().getMask_word_2()));
        ...
        //At this point we have the two top structures, so now we read in the individual data type records.
...
//                    System.out.println("Total Rays of var:" + cv + " is :" + sweeplist.get(currSweep).getIdh_list().get(cv).getRays_present());

                }//Data Headers for current sweep finished.
                datadecoder = new DataDecoderBuffer(dis, getTempByteBuffer());
                /*By Ray*/ for (int raycount = 0; raycount < sweeplist.get(currSweep).getIdh_list().get(1).getRays_present(); raycount++) {
                    currRay = new Ray();
...
//                        System.out.println("Reading(" + sraycount++ + ")Variable " + cv + "of type " + sweeplist.get(currSweep).getIdh_list().get(cv).getData_type());
//                        System.out.println("CurrDataRay elv:" + currDataRay.getRayheader().getBegin_elv());
                        if (sweeplist.get(currSweep).getIdh_list().get(cv).getBits_per_bin() == 160) {
                            ...
                            datadecoder.getData(sweeplist.get(currSweep).getIdh_list().get(cv).getBits_per_bin() / 8 * numbins).get(byteli); //Yeah I know thats ugly.
