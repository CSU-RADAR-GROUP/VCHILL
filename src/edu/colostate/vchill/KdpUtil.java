...
public class KdpUtil {
    ...
    public static double[] calculateKDP(final double[] phi, final double[] dbz, final double[] rhohv, final double startRange, final double gateWidth) {
        ...
        { //find the 1st and last 4 good gates; pad lead-in/-out with average
            ...
            if (first != null && last != null && first.start != last.start) { //fill in bad spots with weighted average of surrounding good data
                Run prev = first;
     ...
     */
    private static Run findNextGoodRun(final int index, final double[] phidp, final double[] snr, final double[] rhohv) { //find the 1st 4 good gates
        double total = 0;
        int start = index;
